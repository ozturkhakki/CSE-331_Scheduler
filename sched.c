...
asmlinkage void schedule(void)
{
	...
	int this_cpu, c;
	/* our variables */
	int sum_priority, user_arr_count, i;
	struct baseper_struct user_arr[64];
...
repeat_schedule:
	...
	if (unlikely(!c)) {
		...
		if (sched_type == SCHED_CSE331) {
			...
			list_for_each(tmp, &runqueue_head) {
				...
				for (i = 0; i < user_arr_count; ++i) 
					if (user_arr[i].uid == p->user->uid) 
						break;

				/* if we reached the end without finding one */
				if (i == user_arr_count) {
					user_arr[user_arr_count].uid = p->user->uid;
					user_arr[user_arr_count].count = 1;
					/* baseper at this point holds priority, but will change */
					user_arr[user_arr_count].baseper = p->user->priority;
					user_arr_count++;
					sum_priority += p->user->priority;
				}
				else {
					user_arr[i].count++;
				}
			...
			for (i = 0; i < user_arr_count; ++i)
				user_arr[i].baseper = ((100 / sum_priority) * user_arr[i].baseper) / user_arr[i].count;
		}
		/* Our algorithm */
		...
		for_each_task(p) {
			...
			/* Our algorithm */
			else if (sched_type == SCHED_CSE331) {
				for (i = 0; i < user_arr_count; ++i)
					if (user_arr[i].uid == p->user->uid)
						break;

				p->counter = user_arr[i].baseper + NICE_TO_TICKS(p->nice);
			}
			/* Our algorithm */
		}
